...
public interface CompraRepository extends JpaRepository<Compra, Long> {
    ...
    @Query("SELECT c FROM Compra c WHERE c.usuario.id = :usuarioId ORDER BY c.oferta.inicio DESC")
    List<Compra> findAllByUsuarioId(@Param("usuarioId") UUID usuarioId);

    @Query("SELECT c FROM Compra c WHERE c.usuario.id = :usuarioId AND c.oferta.status = :status ORDER BY c.oferta.inicio DESC")
    List<Compra> findAllByUsuarioIdWhereStatusConcluido(@Param("usuarioId") UUID usuarioId,
            @Param("status") OfertaStatus status);

    @Query("SELECT c FROM Compra c WHERE c.usuario.id = :usuarioId AND c.oferta.status = :status ORDER BY c.oferta.inicio DESC")
    List<Compra> findAllByUsuarioIdWhereStatusEmAdamento(@Param("usuarioId") UUID usuarioId,
            @Param("status") OfertaStatus status);
    ...
    Optional<Compra> findByUsuarioAndOferta(Usuario usuario, com.fatec.destino.model.pacote.oferta.Oferta oferta);
}
